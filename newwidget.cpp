...
void NewWidget::initNetworkTask()
{
    ...
    connect(this,SIGNAL(sig_JumpFreq(double,double,quint32 /*t0*/,quint32)),worker,SLOT(onJumpFreq(double,double,quint32 /*t0*/,quint32)));
#endif
    mWorkerThread.start();
}
...
void NewWidget::on_m_pBtnStartScan_clicked()
{
    ...
    QFile file(QApplication::applicationDirPath()+"\\data\\"+ui->m_pComBoxDataList->currentText()+"\\up_conversion.txt");//·����Ҫ�޸ĳ�ʵ��ʹ��·��
    if(!file.open(QIODevice::ReadOnly))
    {
    ...
    sendObject.insert("power_start",QString::number(ui->m_pDSBStartPower->value()));//����ʵ��Ϊ-dBm
    sendObject.insert("power_step",QString::number(ui->m_pDSBBujinpower->value()));//����ʵ��Ϊ-dBm

    sendObject.insert("power_stop",QString::number(ui->m_pDSBEndPower->value()));//����ʵ��Ϊ-dBm
...
void NewWidget::on_pushButton_clicked()
{
    ...
    emit sig_JumpFreq(ui->m_pDSpinBoxDDS1Freq->value(),
                                    ...
                                    ui->m_pDoubleSpinBoxT0->value()*100000000,
                                    ui->m_pDoubleSpinBoxT1->value()*100000000);
}
